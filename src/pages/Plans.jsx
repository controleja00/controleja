  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: "#fbfaf8" }}>
      <nav className="sticky top-0 z-50 border-b" style={{ background: "rgba(244,246,250,0.97)", borderColor: "#efefef", backdropFilter: "blur(12px)" }}>
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/register" className="text-sm font-bold px-4 py-2 rounded-xl text-white shadow-md" style={{ background: "#004038" }}>Criar conta grátis</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="max-w-3xl mx-auto px-4 pt-16 pb-8 text-center">
        ...
        <p className="text-lg" style={{ color: "#3d3e45" }}>Comece grátis, sem cartão. Faça upgrade quando precisar de mais.</p>
        <div className="flex flex-wrap gap-4 justify-center mt-5 text-sm" style={{ color: "#3d3e45" }}>
          ...
          {plans.map(plan => (
            <div key={plan.name} className="relative rounded-2xl p-7 flex flex-col bg-white" style={{ border: plan.highlight ? "2px solid #004038" : "1.5px solid #efefef", boxShadow: plan.highlight ? "0 8px 32px rgba(27,47,85,0.15)" : "none" }}>
              ...
              <div className="mb-5">
                ...
                <p className="text-sm mt-1" style={{ color: "#3d3e45" }}>{plan.desc}</p>
              </div>
              <div className="mb-6">
              ...
              <Link to={plan.ctaTo} className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl font-bold text-sm transition-colors" style={plan.highlight ? { background: "#004038", color: "#FFFFFF" } : { border: "2px solid #efefef", color: "#004038", background: "#FFFFFF" }}>
          ...
          {[
            ...
            <details key={faq.q} className="group rounded-xl" style={{ border: "1.5px solid #efefef" }}>
              <summary className="px-5 py-4 text-sm font-semibold cursor-pointer list-none flex items-center justify-between hover:bg-gray-50 rounded-xl transition-colors" style={{ color: "#111917" }}>
                {faq.q}<span className="group-open:rotate-180 transition-transform text-xs ml-3 shrink-0" style={{ color: "#6f7073" }}>▾</span>
              </summary>
              <p className="px-5 pb-4 text-sm leading-relaxed" style={{ color: "#3d3e45" }}>{faq.a}</p>
            </details>
          ))}
        </div>
      </section>

      <footer style={{ background: "#0f161e" }} className="py-8">
