...
export default function Register() {
  ...
  useEffect(() => { document.title = "Criar conta | ControleJá"; }, []);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8" style={{ background: "#F4F6FA" }}>
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl p-7 shadow-sm" style={{ background: "#FFFFFF", border: "1.5px solid #D7D9DE" }}>
          {step === "form" ? (
            <>
              <h1 className="text-2xl font-black mb-1" style={{ color: "#101828" }}>Crie sua conta</h1>
              <p className="text-sm mb-6" style={{ color: "#667085" }}>Grátis para começar, sem cartão de crédito</p>
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: "#101828" }}>Senha * <span className="font-normal text-xs" style={{ color: "#A8ABB3" }}>(mínimo 6 caracteres)</span></label>
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: "#101828" }}>Confirmar senha *</label>
              ...
              <button onClick={() => base44.auth.loginWithProvider("google", "/onboarding")} className="w-full h-11 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition-colors hover:bg-gray-50" style={{ border: "1.5px solid #D7D9DE", background: "#FFFFFF", color: "#101828" }}>
              ...
              <p className="text-center text-sm mt-5" style={{ color: "#667085" }}>
                Já tem conta? <Link to="/login" className="font-bold hover:underline" style={{ color: "#1B2F55" }}>Entrar</Link>
              </p>
            </>
          ) : (
            <>
              <div className="text-center mb-5">
                <div className="h-12 w-12 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "#EFF6FF" }}>
                  <svg className="h-6 w-6" style={{ color: "#1B2F55" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <h1 className="text-2xl font-black mb-1" style={{ color: "#101828" }}>Confirme seu e-mail</h1>
                <p className="text-sm" style={{ color: "#667085" }}>Enviamos um código de verificação para</p>
                <p className="text-sm font-bold mt-0.5" style={{ color: "#101828" }}>{email}</p>
              </div>
              ...
              <form onSubmit={handleVerify} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: "#101828" }}>Código de verificação</label>
                  <input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="000000" maxLength={6} inputMode="numeric"
                    ...
                    style={{ border: "1.5px solid #D7D9DE", background: "#F4F6FA", color: "#1B2F55" }} />
                </div>
                <button type="submit" disabled={loading || otp.length < 4} className="w-full h-11 rounded-xl font-bold text-base text-white disabled:opacity-60" style={{ background: "#1B2F55" }}>
              ...
              <p className="text-center text-sm mt-4" style={{ color: "#667085" }}>
                ...
                <button onClick={handleResend} className="font-bold hover:underline" style={{ color: "#1B2F55" }}>Reenviar</button>
              </p>
              <p className="text-center mt-3">
                <button onClick={() => { setStep("form"); setError(""); setOtp(""); }} className="text-xs hover:underline" style={{ color: "#A8ABB3" }}>Voltar e editar e-mail</button>
              </p>
              <div className="mt-4 rounded-xl p-3 text-xs text-center" style={{ background: "#F4F6FA", color: "#A8ABB3" }}>
