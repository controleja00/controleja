...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await consuobra.auth.register({
        email,
        password,
          ...
          {step === "form" ? (
            ...
            <>
              <div className="text-center py-4">
                ...
                <h1 className="text-2xl font-black mb-2" style={{ color: "#172441" }}>Confirme seu e-mail</h1>
                <p className="text-sm mb-2" style={{ color: "#424c62" }}>
                  Enviamos um link de confirmação para <strong>{email}</strong>.
                </p>
                <p className="text-xs mb-6" style={{ color: "#778096" }}>
                  Depois de confirmar, volte para entrar e concluir o onboarding da Consuobra.
                </p>
                <Link to="/login" className="w-full h-11 rounded-xl font-bold text-base text-white flex items-center justify-center" style={{ background: "#1f3258" }}>
                  Ir para o login
                </Link>
                <button onClick={() => { setStep("form"); setError(""); }} className="text-xs hover:underline mt-4" style={{ color: "#778096" }}>
