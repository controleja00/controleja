...
export default function Login() {
  ...
  const getSafeNext = () => {
    ...
    if (!next || next.includes("\\")) return "/dashboard";
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await consuobra.auth.loginViaEmailPassword(email, password);
      redirect();
    } catch {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8" style={{ background: "#f6f8fc" }}>
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl p-7" style={{ background: "#FFFFFF", border: "1.5px solid #e1e5ed" }}>
          <h1 className="text-2xl font-black mb-1" style={{ color: "#172441" }}>Bem-vindo de volta</h1>
          <p className="text-sm mb-6" style={{ color: "#424c62" }}>Entre para continuar no Consuobra</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm font-semibold" style={{ color: "#172441" }}>Senha</label>
                <Link to="/forgot-password" className="text-xs font-medium hover:underline" style={{ color: "#435b89" }}>Esqueci minha senha</Link>
              </div>
          ...
          <p className="text-center text-sm mt-5" style={{ color: "#424c62" }}>
            Não tem conta?{" "}
            <Link to="/register" className="font-bold hover:underline" style={{ color: "#1f3258" }}>Criar conta grátis</Link>
          </p>
        </div>
