...
export default function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 shadow-2xl safe-area-inset-bottom" style={{ background: "#13233F" }}>
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                <tab.icon
                  className={cn("h-5 w-5 transition-all", active ? "text-white" : "text-white/40")}
                  strokeWidth={active ? 2.5 : 1.75}
                />
              </div>
              <span className={cn("text-[10px] font-semibold transition-all", active ? "text-white" : "text-white/30")}>
                {tab.label}
              </span>
            </Link>
          );
